...
select ROW (lastname) from employees;
...
select departments SET budget = budget - budget * 0.1 where budget = (select budget from departments order by budget asc limit 1) returning *;

update employees set department = 14 where department = 77 returning *;

delete from employees as  where e.department = 14;
